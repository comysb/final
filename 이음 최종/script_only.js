...
══════════════════════════════════════════════════════════ */
  /* ── 실시간 세션 관리 (Phase 7) ────────────────────────── */
  const WS_URL   = 'ws://127.0.0.1:8001/ws/rehab/live';
  const CHUNK_SR = 16000;

  const TASK_TYPE_MAP = {
    3:  'sustained_phonation',
    4:  'pitch_glide_up',
    5:  'pitch_glide_down',
    6:  'pitch_control',
    7:  'loudness_control',
  ...
  class LiveRecorder {
    constructor(taskType, onLive, onFinal) {
    ...
    async start() {
      try {
        // 16kHz 모노 + 에코캔슬 명시적 요청 (미지원 시 브라우저 기본값으로 자동 폴백)
        this.stream = await navigator.mediaDevices.getUserMedia({
          audio: {
            sampleRate:        { ideal: CHUNK_SR },
        ...
        console.log('[LiveRecorder] 마이크 설정:', JSON.stringify(track.getSettings()));
      } catch (e) {
        console.error('[LiveRecorder] 마이크 권한 거부:', e);
        alert('마이크를 사용할 수 없습니다. 브라우저 설정에서 마이크 권한을 허용해주세요.');
        return false;
      }
      ...
      this.ws.onopen = () => {
        console.log('[LiveRecorder] WS 열림, open 메시지 전송');
        this.ws.send(JSON.stringify({
          type: 'open', task_type: this.taskType, session_id: this.sessionId,
        }));
      };

      this.ws.onclose = (ev) => {
        console.warn('[LiveRecorder] WS 닫힘, code=', ev.code, ev.reason);
      };

      this.ws.onerror = (e) => {
        console.error('[LiveRecorder] WS 에러:', e);
      };
      ...
      this.ws.onmessage = (ev) => {
        ...
        if (msg.type === 'opened') {
          console.log('[LiveRecorder] 서버로부터 opened 확인, worklet 시작');
          this._startAudioWorklet();
        } else if (msg.type === 'live')  { this.onLive(msg.data); }
        else if (msg.type === 'final')   { this.onFinal(msg.data); }
        else if (msg.type === 'error')   { console.error('[WS]', msg.message); }
      };

      return true;
    }

    async _startAudioWorklet() {
      try {
        this.ctx = new AudioContext({ sampleRate: CHUNK_SR });
        // Chrome에서 AudioContext는 user gesture 없이 suspended 상태일 수 있음 → 명시적 resume
        if (this.ctx.state === 'suspended') {
          await this.ctx.resume();
          console.log('[LiveRecorder] AudioContext resumed');
        }
        const actualSR = this.ctx.sampleRate;
        if (actualSR !== CHUNK_SR) {
          console.warn(`[LiveRecorder] AudioContext sampleRate=${actualSR} (요청=${CHUNK_SR}). chunk_processor가 자동 다운샘플링합니다.`);
        } else {
          console.log('[LiveRecorder] AudioWorklet 시작 완료, sampleRate=', actualSR);
        }
        const src = this.ctx.createMediaStreamSource(this.stream);
        await this.ctx.audioWorklet.addModule('/static/chunk_processor.js');
        this.processor = new AudioWorkletNode(this.ctx, 'chunk-processor');
        this.processor.port.onmessage = (e) => {
          if (this._stopped) return;
          if (this.ws && this.ws.readyState === WebSocket.OPEN) {
            // chunk_processor.js가 항상 16kHz Int16 PCM 1600샘플을 전송
            this.ws.send(e.data);
          }
        };
        // ✅ processor는 마이크 소스만 연결 — destination 연결 제거 (스피커 피드백 방지)
        src.connect(this.processor);
      } catch (e) {
        console.error('[LiveRecorder] Worklet 시작 실패:', e);
      }
    }
    ...
    stop(promptText) {
      ...
      // ── 최종 평가 트리거: close 메시지를 먼저 전송한 뒤 WS를 닫는다 ──
      const ws = this.ws;
      this.ws = null;
      if (ws && ws.readyState === WebSocket.OPEN) {
        ...
        } catch(e) { console.warn('[LiveRecorder] close 메시지 전송 실패:', e); }
        // final 응답 수신 여유를 두고 WS 종료 (서버가 final 전송 후 break)
        setTimeout(() => { try { ws.close(); } catch(e) {} }, 2000);
      }
    }
  }

  window._liveRecorder = null;
  window._isRealtimeSession = false;

  function stopAllRecorders(promptText) {
    if (window._liveRecorder) {
      window._liveRecorder.stop(promptText);
      window._liveRecorder = null;
    }
    window._isRealtimeSession = false;
  }

  function startRealtimeSession(taskId, metrics) {
    stopAllRecorders();
    const taskType = TASK_TYPE_MAP[taskId];
    if (!taskType) return;

    window._isRealtimeSession = true;
    window._liveRecorder = new LiveRecorder(
      taskType,
      (data) => _applyLive(taskId, data, metrics),
      (data) => console.log('[Final Result]', data)
    );
    window._liveRecorder.start();
  }

  function _applyLive(id, data, m) {
    if (window._liveRecorder) window._liveRecorder.lastVoiced = data.voiced;

    // 1. 목소리 인식 애니메이션 (WB) - voiced 일 때만 활성화
    const wbs = document.querySelectorAll('.wb');
    if (data.voiced) {
      wbs.forEach(b => {
    ...
    if (data.current_db !== undefined) {
      const dbText = document.getElementById('loud-db');
      const loudCircle = document.getElementById('loud-circle');
      if (dbText) dbText.textContent = Math.round(data.current_db);
      if (loudCircle && id === 15) {
        // 기본 80px, 70dB일 때 160px 정도로 확장
        const baseSize = 80;
        const scale = Math.max(1, (data.current_db - 45) / 25 * 2);
        const newSize = baseSize * parseFloat(scale.toFixed(2));
        loudCircle.style.width = newSize + 'px';
        loudCircle.style.height = newSize + 'px';

        const readFb = document.getElementById('read-fb');
        if (readFb) {
          if (data.current_db >= 70) {
            readFb.className = 'fb-bar2 good';
            readFb.textContent = '아주 좋습니다!';
            loudCircle.style.background = 'rgba(22, 163, 74, 0.2)';
            loudCircle.style.borderColor = '#16A34A';
            loudCircle.style.boxShadow = '0 0 15px rgba(22, 163, 74, 0.4)';
          } else {
            readFb.className = 'fb-bar2 warn';
            readFb.textContent = '조금 더 크게 읽어보세요';
            loudCircle.style.background = 'rgba(59, 130, 246, 0.2)';
            loudCircle.style.borderColor = '#3B82F6';
            loudCircle.style.boxShadow = 'none';
          }
        }
      }
    }
    ...
    if ([3, 4, 5, 6].includes(id)) {
      if (data.voiced && data.f0 > 0) {
        const hzTxt = document.getElementById('pitch-hz');
        if (hzTxt) hzTxt.textContent = Math.round(data.f0) + ' Hz';
        window._currentLiveF0 = data.f0; // runPitchCycle에서 가져다 쓰도록 전역 저장
      } else {
        window._currentLiveF0 = 0; // 목소리 없으면 F0 초기화
      }
    }

    // 4. 호흡 원 제어 (id 9, 12, 7, 8, 15) 볼륨 게이지 통합
    if ([7, 8, 9, 12, 15].includes(id)) {
      const targetCircle = (id === 15) ? null : (([7, 8].includes(id)) ? document.getElementById('volume-user-circle') : document.getElementById('breath-circle'));
      const targetDbVal  = (id === 15) ? null : (([7, 8].includes(id)) ? document.getElementById('volume-db-val') : null);

      if (targetCircle) {
        let db = data.current_db || 30;
        let effectiveDb = ([9, 12].includes(id)) ? Math.max(0, db - 20) * 1.5 : db;
        let userScale = Math.max(0.2, Math.min(2.5, effectiveDb / 60));

        targetCircle.style.width = ([7, 8].includes(id) ? (60 * userScale) : (60 + 90 * userScale)) + 'px';
        targetCircle.style.height = ([7, 8].includes(id) ? (60 * userScale) : (60 + 90 * userScale)) + 'px';
        if (targetDbVal) targetDbVal.textContent = Math.round(db) + ' dB';

        // 성공 판정 로직
        if ([7, 8].includes(id)) {
          if (db >= 70) {
            targetCircle.classList.add('success');
            window._liveProgress = data.success_hold_sec || (window._liveProgress + 0.1);
            if (window._liveProgress >= 3.0 && document.getElementById('volume-next').style.display === 'none') {
                document.getElementById('volume-next').style.display = 'block';
                const fb = document.getElementById('volume-fb');
                if (fb) {
                    fb.style.display = 'block';
                    fb.textContent = '아주 잘 하셨습니다!';
                }
                if (typeof spawnConfetti === 'function') spawnConfetti();
            }
          } else {
            targetCircle.classList.remove('success');
          }
        }
      }
      ...
      let db = data.current_db || 30;
      if (id === 15) {
        if (typeof READ_DATA !== 'undefined' && READ_DATA.loud[readIdx] && !window._loudDone) {
          const mode = READ_DATA.loud[readIdx].mode;
          // loud 모드에서는 70dB 이상일 때 진행
          if (mode === 'loud' && db >= 70) {
            if (!window._loudProgress) window._loudProgress = 0;
            window._loudProgress += 0.5;
            if (window._loudProgress > 10) {
                window._loudDone = true;
                const rFb = document.getElementById('read-fb');
                if (rFb) { rFb.className = 'fb-bar2 good'; rFb.textContent = '크고 힘찬 목소리 좋습니다! 잠시 후 다음으로 넘어갑니다.'; }
                setTimeout(() => { window._loudProgress = 0; window._loudDone = false; if (typeof readNext === 'function') readNext(); }, 1500);
            }
          }
          // normal 모드에서는 50dB~70dB 사이일 때 진행
          else if (mode === 'normal' && db >= 50 && db < 70) {
            if (!window._loudProgress) window._loudProgress = 0;
            window._loudProgress += 0.5;
            if (window._loudProgress > 10) {
               window._loudDone = true;
               const rFb = document.getElementById('read-fb');
               if (rFb) { rFb.className = 'fb-bar2 good'; rFb.textContent = '잘하셨습니다. 이제 최대한 크게 읽어보세요!'; }
               setTimeout(() => { window._loudProgress = 0; window._loudDone = false; if (typeof readNext === 'function') readNext(); }, 1500);
            }
          }
        }
    ...
    // 대립강세 피드백: 큰 소리가 특정 데시벨 이상 잡히면 강세 부여로 판단 처리
    if (id === 11 && data.current_db > 63) {
      const readFb = document.getElementById('read-fb');
      if (readFb) {
        readFb.className = 'fb-bar2 good';
        readFb.textContent = '강조가 잘 전달되었습니다!';
      }
    }

    // 5. 일반 과제 원형 프로그레스 (3:지속발성, 16:DDK 등)
    if ([3, 16].includes(id)) {
      const ringFill = document.getElementById('ring-fill');
      const ringVal = document.getElementById('ring-val');
      const ringMsg = document.getElementById('ring-msg');
      if (ringFill && ringVal) {
        if (!window._liveProgress) window._liveProgress = 0;
        // 주변 소음 방지: 40dB 이상일 때만 목소리로 인식해 진행률 올림
        if (data.voiced && (data.current_db > 40)) {
            // 서버에서 넘어온 hold_sec(초 단위 누적 발성 시간) 사용 (없을시 로컬 0.1 누적 fallback)
            window._liveProgress = (data.hold_sec !== undefined) ? data.hold_sec : window._liveProgress + 0.1;
        }
        
        let pct = Math.min(100, (window._liveProgress / (m.max || 10)) * 100);
        ringVal.textContent = (m.mode === 'time' || m.mode === 'ddk') ? window._liveProgress.toFixed(1) : Math.round(window._liveProgress);
        ringFill.style.strokeDashoffset = 553 - (pct / 100 * 553);
        
        if (pct >= 100 && !window._taskDone) {
            window._taskDone = true;
            ringFill.style.stroke = '#16A34A';
            if (ringMsg) {
                ringMsg.textContent = '목표 달성! 아주 잘 하셨습니다.';
                ringMsg.style.color = '#16A34A';
            }
            // 완료 화면 전환 (voice-done 오버레이 표시)
            const done = VOICE_DONE_MSG[id] || {title:'참 잘하셨어요!', sub:'성공적으로 과제를 마쳤습니다.'};
...
const FOCUS_COMBOS = {
  ...
  "normal-mild": {
    ...
    results:[
      ...
      {e:"DDK",n:"DDK 직접훈련",met:"5.9회/초 · 94%",      ch:"↑+15%",   up:true},
...
const SEV_WORDS_RAW = ['나무','목도리','꽃','김밥','바지','사탕','풍선','국자','토끼','코끼리',
  '해바라기','연필','호랑이','라면','냉장고','단추','곰','가방','책상','자동차',
  '빨간색','그네','기차','접시','로봇','짜장면','포크'];
...
function toggleSevTpRec() {
  ...
  if (!tpState.recording) {
    ...
    // 1.5초마다 다음 단어로 스크롤
    scrollTeleprompter();
    tpState.scrollTimer = setInterval(scrollTeleprompter, 1500);
  } else {
    stopTpRec();
    showSevRecDone();
  }
...
function showSevComplete() {
  ...
  // 초기 상태
  document.getElementById('sev-analyzing').style.display = 'flex';
  document.getElementById('sev-training-ready').style.display = 'none';
  document.getElementById('sev-complete-btn').style.opacity = '0.5';
  document.getElementById('sev-complete-btn').style.pointerEvents = 'none';
  // 2.5초 후 분석 완료 → 맞춤 훈련 안내
  setTimeout(() => {
    document.getElementById('sev-analyzing').style.display = 'none';
    document.getElementById('sev-training-ready').style.display = 'block';
    document.getElementById('sev-complete-btn').style.opacity = '1';
    document.getElementById('sev-complete-btn').style.pointerEvents = 'auto';
  }, 2500);
}
...
const METRICS = {
  ...
  16: {lbl:"DDK 속도",      u:"회/초",   goal:"목표: 4.0회/초 이상",      max:4.0, mode:'ddk'},
...
function loadExercise() {
  ...
  // 영상 매핑
  const VIDEO_MAP = {
    1:  '/static/videos/1. 발성의 시작, 바른 자세 교정 훈련_1080p_caption.mp4',
    2:  '/static/videos/2. 구강 운동 가이드 (통합 수정본)_1080p_caption.mp4',
    3:  '/static/videos/3. 최대발성지속 시범 및 안내_1080p_caption.mp4',
    4:  '/static/videos/4.음도 글라이딩 정석 시연 (흰 배경)_1080p_caption.mp4',
    5:  '/static/videos/5. 음도 글라이딩 완벽 하강 시연 (흰 배경)_1080p_caption.mp4',
    6:  '/static/videos/6. 정교한 소리 조절, 피치 조절 훈련 (흰 배경)_1080p_caption.mp4',
    7:  '/static/videos/7. 볼륨업 (크게 발성) 훈련 안내_1080p_caption.mp4',
    8:  '/static/videos/8. 성문폐쇄 (Voice Onset) 훈련 안내_1080p_caption.mp4',
    10: '/static/videos/10. 정확한 발음의 기초, 대조 훈련 (최소 대립쌍)_1080p_caption.mp4',
    11: '/static/videos/11. 의미를 전달하는 힘, 대립강세 훈련 (흰 배경)_1080p_caption (1).mp4',
    12: '/static/videos/12_1. 발성의 기초, 복식 호흡 훈련 (흰 배경)_1080p_caption.mp4',
    16: '/static/videos/16. 조음 민첩성 향상, DDK 직접 훈련 (_퍼터커_)_1080p_caption.mp4',
  };
  ...
  document.getElementById('phase-volume').style.display = 'none';
...
function startTask() {
  ...
  stopRecTimer();
  window._liveProgress = 0; 
  window._taskDone = false;
  window._loudDone = false;
  window._loudProgress = 0;

  // 1. 모든 이전 단계 및 완료 화면 숨기기 (UI 누수 방지)
  ...
  const containers = ['phase-b','phase-pitch','phase-volume','phase-b-read','phase-c','phase-breath','voice-done'];
  ...
  // 2. 실시간 세션 시작 (Phase 7)
  const isLive = !!TASK_TYPE_MAP[id];
  if (isLive) {
    startRealtimeSession(id, getLvl(id));
  }
  ...
  // 음도올리기/내리기/피치조절/지속발성 전용
  const PITCH_EX = [3, 4, 5, 6];
  ...
  if (VOLUME_EX.includes(id)) {
    document.getElementById('phase-volume').style.display = 'block';
    startVolumeTask(id, getLvl(id));
    return;
  }
  ...
  if (ex.t === 'self') {
    ...
    // ... self data logic (omitted for brevity, keeping existing)
    ...
    checksEl.innerHTML = sd.checks.map(txt => `<div class="self-check-item"><div class="self-check-dot"></div><div class="self-check-label">${txt}</div></div>`).join('');
    let selfSec = 0;
    ...
    // 읽기 과제 전용 Phase B-read (startReadTask 내에서 startRealtimeSession 호출됨)
    startReadTask(id);
  } else {
    // 일반 음성 과제 (지속발성, 성량 등)
    const m = Object.assign({}, METRICS[id] || METRICS['mini']);
    const lvData = getLvl(id);
    if (lvData && lvData.goalTxt) { m.goal = lvData.goalTxt; m.max = lvData.max; }
    ...
    const VOICE_INIT_FB = { 3:'소리를 길~게 이어보세요!', 7:'힘차게 크게 소리 내보세요!', 8:'강하게 "이!" 시작해보세요!', 16:'"/퍼터커/" 반복해보세요!' };
...
function startBreathGuide(id) {
  ...
  function runCycle(r) {
    if (r >= totalReps) {
      stopAllRecorders();
      guide.textContent = '참 잘하셨어요!';
      sub.textContent = '호흡이 한결 편안해졌습니다.';
...
function getPitchPoint(progress, type) {
  ...
  let y;
  if (type === 'up') {
    // 1/4원 (아래로 휘어진 상태: 중심을 좌상단에 두고 밑으로 내려오는 원의 사분면)
    // 원방정식 y = 1 - sqrt(1 - x^2)을 적용하여 하단으로 볼록한 형태 구성
    const arc = 1 - Math.sqrt(1 - Math.pow(progress, 2));
    y = (H - pad) - arc * (H - pad * 2);
  } else if (type === 'down') {
    // 1/4원 (위로 휘어진 상태: 중심을 우하단에 둔 원의 사분면)
    // 원방정식 y = sqrt(1 - x^2)을 적용하여 상단으로 볼록한 형태 구성
    const arc = Math.sqrt(1 - Math.pow(progress, 2));
    y = pad + (1 - arc) * (H - pad * 2);
  } else if (type === 'flat') {
    // 지속발성: 일직선 수평 가이드
    y = H / 2;
  } else {
    // 피치조절: 파도형 (주기 1.5, 위로 올라갈 때 y를 뺌)
    y = (H / 2) - Math.sin(progress * Math.PI * 3) * 50;
  }
  return { x, y };
}
...
let pitchStartTo = null; // 타임아웃 방지용

function startPitchGuide(id) {
  curPitchId = id;
  pitchTotalReps = 3;
  const type = id === 3 ? 'flat' : id === 4 ? 'up' : id === 5 ? 'down' : 'wave';
  ...
  const endP = getPitchPoint(1, type);

  // 선 양 끝단 앵커 동그라미 그리기
  document.getElementById('pitch-start-circle').setAttribute('cx', startP.x);
  document.getElementById('pitch-start-circle').setAttribute('cy', startP.y);
  document.getElementById('pitch-end-circle').setAttribute('cx', endP.x);
  document.getElementById('pitch-end-circle').setAttribute('cy', endP.y);

  // 라벨 문자 '아'를 선 끝 동그라미들 위에 위치시키기 (동적 % 할당 + transform)
  const loLbl = document.querySelector('.pitch-sound-lbl.lo');
  const hiLbl = document.getElementById('pitch-hi-lbl');
  
  loLbl.style.left = `${(startP.x / 320) * 100}%`;
  loLbl.style.top = `calc(${(startP.y / 200) * 100}% - 22px)`;
  loLbl.style.bottom = 'auto'; loLbl.style.right = 'auto';
  loLbl.style.transform = 'translate(-50%, -50%)';

  if (id === 6) {
    hiLbl.style.display = 'none';
  } else {
    hiLbl.style.display = 'block';
    hiLbl.style.left = `${(endP.x / 320) * 100}%`;
    hiLbl.style.top = `calc(${(endP.y / 200) * 100}% - 22px)`;
    hiLbl.style.bottom = 'auto'; hiLbl.style.right = 'auto';
    hiLbl.style.transform = 'translate(-50%, -50%)';
  }
  ...
  document.getElementById('phase-pitch').style.display = 'block';
  document.getElementById('pitch-hz').style.display = 'block';

  stopPitchGuide();
  pitchStartTo = setTimeout(() => runPitchCycle(0), 800);
}
...
function runPitchCycle(r) {
  ...
  const type = curPitchId === 4 ? 'up' : curPitchId === 5 ? 'down' : curPitchId === 3 ? 'flat' : 'wave';
  document.getElementById('pitch-rep').textContent = (r + 1) + ' / ' + pitchTotalReps + '회';
  window._pitchActualPath = []; // 사용자 실제 경로 초기화
  ...
  const msgs = { 3: '일정한 음으로 소리 내세요', 4: '소리를 내면서 올려보세요', 5: '소리를 내면서 내려보세요', 6: '목표 음에 맞춰보세요' };
  ...
  pitchIv = setInterval(() => {
    // 목소리 감지될 때만 진행!
    if (!window._isRealtimeSession || window._currentLiveF0 > 0 || (window._liveRecorder && window._liveRecorder.lastVoiced)) {
      progress += 0.0035; // 전체 약 5.5초 정도가 되도록 부드럽게 (20ms interval 기준 280틱)
    } else {
      document.getElementById('pitch-msg').textContent = '소리를 내면 시작됩니다...';
      document.getElementById('pitch-msg').style.color = 'var(--t1)';
      document.getElementById('pitch-dot').style.fill = '#CBD5E1';
      return; // 진행 중단
    }
    ...
    // 실제 사용자의 음도(Hz) 반영 (Y축 매핑)
    // 경로 위의 공(pitch-dot)은 리얼 F0에 취해 타겟에서 벗어날 수 있음
    if (window._currentLiveF0 > 0) {
      const liveF0 = window._currentLiveF0;
      // SVG Y 매핑: 80Hz(아래) ~ 350Hz(위), H=200, pad=30
      const minF0 = 70, maxF0 = 360;
      const H = 200, pad = 30;
      const liveY = (H - pad) - ((liveF0 - minF0) / (maxF0 - minF0)) * (H - pad * 2);
      const clampedY = Math.max(pad, Math.min(H - pad, liveY));

      document.getElementById('pitch-dot').setAttribute('cy', clampedY);
      document.getElementById('pitch-dot-pulse').setAttribute('cy', clampedY);
      // X는 progress를 따라감
      const guideP = getPitchPoint(progress, type);
      document.getElementById('pitch-dot').setAttribute('cx', guideP.x);
      document.getElementById('pitch-dot-pulse').setAttribute('cx', guideP.x);

      document.getElementById('pitch-hz').style.left = (() => {
        const canvas = document.getElementById('pitch-canvas');
        const svgRect = canvas.getBoundingClientRect();
        return (guideP.x * (svgRect.width / 320)) + 'px';
      })();
      document.getElementById('pitch-hz').style.top = (() => {
        const canvas = document.getElementById('pitch-canvas');
        const svgRect = canvas.getBoundingClientRect();
        return (clampedY * (svgRect.height / 200)) + 'px';
      })();
      document.getElementById('pitch-hz').textContent = Math.round(liveF0) + ' Hz';
      
      // 목표와의 오차 체크 (성공 여부 시각화)
      const dy = Math.abs(clampedY - guideP.y);
      if (dy < 25) {
        document.getElementById('pitch-dot').style.fill = '#16A34A';
        document.getElementById('pitch-msg').textContent = '잘 맞추고 있어요!';
        document.getElementById('pitch-msg').style.color = 'var(--green)';
      } else {
        document.getElementById('pitch-dot').style.fill = '#EF4444'; // 빨간색 = 크게 벗어남
        document.getElementById('pitch-msg').textContent = '피치를 \uc870 더 ' + (curPitchId === 4 ? '올려' : '내려') + '보세요';
        document.getElementById('pitch-msg').style.color = '#EF4444';
      }
    } else {
      // voiced 없으면 편 발행 X → 공은 가이드 위 (progress 기준)
      const guideP = getPitchPoint(progress, type);
      document.getElementById('pitch-dot').setAttribute('cx', guideP.x);
      document.getElementById('pitch-dot').setAttribute('cy', guideP.y);
      document.getElementById('pitch-dot-pulse').setAttribute('cx', guideP.x);
      document.getElementById('pitch-dot-pulse').setAttribute('cy', guideP.y);
      document.getElementById('pitch-dot').style.fill = '#CBD5E1';
    }
...
function stopPitchGuide() {
  if (pitchStartTo) { clearTimeout(pitchStartTo); pitchStartTo = null; }
  if (pitchIv) { clearInterval(pitchIv); pitchIv = null; }
}
...
const READ_DATA = {
  ...
  // 라우드리딩 문장 목록 (#15) - 2페이즈 버전
  loud: [
    {text:"오늘 날씨가 참 좋네요.", mode:"normal", inst:"지금 문장을 '보통 크기'로 읽어보세요."},
    {text:"오늘 날씨가 참 좋네요.", mode:"loud",   inst:"같은 문장을 '최대한 크게' 읽으세요!"},
    {text:"병원에 가야 합니다.",   mode:"normal", inst:"지금 문장을 '보통 크기'로 읽어보세요."},
    {text:"병원에 가야 합니다.",   mode:"loud",   inst:"같은 문장을 '최대한 크게' 읽으세요!"},
  ],
  // 대조훈련 최소대립쌍 (#10)
  pair: [
  ...
  // 대립강세 (#11) - 문장별 3가지 강조 버전 분산
  stress: [
    {text:"나는 사과를 먹었다.", target:"나는",   syl:["나","는"," ","사","과","를"," ","먹","었","다","."]},
    {text:"나는 사과를 먹었다.", target:"사과를", syl:["나","는"," ","사","과","를"," ","먹","었","다","."]},
    {text:"나는 사과를 먹었다.", target:"먹었다.", syl:["나","는"," ","사","과","를"," ","먹","었","다","."]},
    {text:"오늘 병원에 갔다.",   target:"오늘",   syl:["오","늘"," ","병","원","에"," ","갔","다","."]},
    {text:"오늘 병원에 갔다.",   target:"병원에", syl:["오","늘"," ","병","원","에"," ","갔","다","."]},
    {text:"오늘 병원에 갔다.",   target:"갔다.",   syl:["오","늘"," ","병","원","에"," ","갔","다","."]},
  ],
};
...
function startReadTask(id) {
  ...
  if (readType === 'slow' || readType === 'stress') {
    ...
    loadSentence();
    // 첫 문장 포함 항상 데모 시작 (첫 문장도 포함하여 예시가 먼저 지나가야 함)
    if (!window._isRealtimeSession) startSylTimer();
    else startSylTimer(); // 실시간이라도 데모는 항상 실행
  } else if (readType === 'loud') {
    readList = READ_DATA.loud;
    document.getElementById('sent-area').style.display = 'block';
    document.getElementById('loud-area').style.display = 'block';
    loadSentence();
    if (!window._isRealtimeSession) startLoudTimer();
  ...
  document.getElementById('read-fb').textContent = '읽으시면 목소리가 인식됩니다';
...
function loadSentence() {
  ...
  if (readType === 'stress') {
    // 강세 과제: 특정 목표 단어 강조 (파란색 + 위에 점 + 조금 크게)
    const target = item.target;
    const highlighted = item.text.replace(target,
      `<span style="color:#3B82F6; font-weight:900; font-size:1.25em;
      ...
      ><span style="position:absolute;top:0;left:50%;transform:translateX(-50%);
        width:7px;height:7px;background:#3B82F6;border-radius:50%;display:block;"></span>${target}</span>`);
    document.getElementById('sent-text').innerHTML =
       `<div style="font-size:14px; color:var(--t3); margin-bottom:8px;">아래 파란 단어를 특히 강조하세요:</div>` +
       `<div style="font-size:30px; font-weight:700; line-height:2;">${highlighted}</div>`;
  } else if (readType === 'loud') {
    // 크게 읽기: 단계별 지시문(보통/최대한) 반영
    document.getElementById('sent-text').textContent = item.text;
    const instMsg = document.getElementById('loud-inst-msg');
    if (instMsg) {
      instMsg.textContent = item.inst || '최대한 크게 말하세요!';
      instMsg.style.color = (item.mode === 'loud' ? 'var(--primary)' : 'var(--t3)');
      // 지시문 옆에 모드 표시 (보통/크게)
      const modeLbl = (item.mode === 'normal' ? '[1단계: 보통 크기]' : '[2단계: 최대한 크게]');
      instMsg.innerHTML = `<span style="font-weight:900; color:var(--primary); margin-right:8px;">${modeLbl}</span>` + instMsg.textContent;
    }
    const userCirc = document.getElementById('volume-user-circle-read');
    if (userCirc) userCirc.classList.remove('success');
  } else {
    // 슬로우: 음절 하이라이트 span 생성
    if (item.syl) {
...
function selectPair(which) {
  ...
  document.getElementById('read-fb').textContent = '정확하게 구별했어요! (선택: ' + correct + ')';
}
...
/* 슬로우 리딩 음절 하이라이트 타이머 (듀얼 바) */
// 타겟 바가 먼저 진행된 후 (waitTarget=true), 그 다음 유저가 발성 (waitTarget=false)
let waitTarget = true;
  ...
  sylIv = setInterval(() => {
    ...
    if (waitTarget) {
      ...
      if (sylIdx >= item.syl.length) {
        // 타겟 완료. 이제 파란색은 전부 지우고 사용자 차례로 전환
        waitTarget = false;
        document.getElementById('read-fb').className = 'fb-bar2 good';
        document.getElementById('read-fb').textContent = '준비 완료! 이제 같은 속도로 읽어보세요.';
        // 데모가 끝났으니 파란색 바(cur 클래스) 일괄 제거
        const allSyls = document.querySelectorAll('.syl');
        allSyls.forEach(s => s.classList.remove('cur'));
      } else {
    ...
    // 2. 초록색 바 (사용자 차례)
    if (!waitTarget && window._isRealtimeSession && window._liveRecorder && window._liveRecorder.lastVoiced) {
        sylIdxUser++;
        const uCur = document.getElementById('syl-' + sylIdxUser);
        if (uCur) {
            uCur.classList.add('user-cur');
            const fb = document.getElementById('read-fb');
            // 여기서는 이미 데모가 지나간 후 본인이 읽는 속도이므로 경고 기준을 완화
            if (sylIdxUser > Math.floor(sylIdx * 1.5)) { 
                uCur.classList.add('too-fast');
                fb.textContent = '너무 빨라요! 천천히 읽으세요.';
                fb.className = 'fb-bar2 warn';
                waitTarget = false; // 속도가 안 맞으면 빨간색
            } else {
                uCur.classList.remove('too-fast');
                fb.textContent = '적절한 속도입니다.';
                fb.className = 'fb-bar2 good';
            }
        }
    }

    if (!waitTarget && sylIdxUser >= item.syl.length) {
      clearInterval(sylIv); sylIv = null;
      document.getElementById('read-fb').className = 'fb-bar2 good';
      document.getElementById('read-fb').textContent = '잘 읽었어요! 다음 페이지로 이동하세요';
    }
  }, 450);
}
...
function startTapTimer() {
  ...
  const item = readList[readIdx];
  tapIv = setInterval(() => {
    // 이전 박자점 색상 리셋 (파란색/빨간색 제거, 기본 회색으로)
    ...
    // 맨 끝에서 0으로 돌아갈 때 마지막 것도 초기화
    if (tapBeat === 0) {
      const last = document.getElementById('td-' + (item.beats - 1));
      if (last) { last.classList.remove('on'); last.style.backgroundColor = ''; last.style.transform = ''; }
    }
...
function startVolumeTask(id, m) {
  curVoiceId = id;
  curVoiceM  = m;
  window._liveProgress = 0;
  
  const userCirc = document.getElementById('volume-user-circle');
  const dbVal    = document.getElementById('volume-db-val');
  const msg      = document.getElementById('volume-msg');
  
  userCirc.classList.remove('success');
  userCirc.style.width = '60px';
  userCirc.style.height = '60px';
  dbVal.textContent = '0';
  msg.textContent = (id === 8 ? '성대를 닫으며 "이!"' : '최대한 크게 "아!"');
  msg.style.color = 'var(--t1)';
  
  document.getElementById('volume-next').style.display = 'none';
  document.getElementById('volume-fb').style.display = 'none';
}

function handleTapDown(e) {
  if (e) e.preventDefault();
  const btn = document.getElementById('tap-big-button');
  btn.classList.add('hit');
  
  // 파란색 / 빨간색 즉각 피드백 적용 (버튼 및 박자 점)
  const curDot = document.getElementById('td-' + tapBeat);
  const isHit = (curDot && curDot.classList.contains('on'));
  
  if (isHit) {
    btn.style.backgroundColor = '#3B82F6';
    btn.style.borderColor = '#2563EB';
    document.getElementById('read-fb').className = 'fb-bar2 good';
    document.getElementById('read-fb').textContent = '정확한 타이밍입니다!';
    if (curDot) {
      curDot.style.backgroundColor = '#3B82F6';
      curDot.style.transform = 'scale(1.5)';
    }
  } else {
    btn.style.backgroundColor = '#EF4444';
    btn.style.borderColor = '#DC2626';
    document.getElementById('read-fb').className = 'fb-bar2 warn';
    document.getElementById('read-fb').textContent = '박자에 맞춰서 눌러보세요';
    if (curDot) {
      curDot.style.backgroundColor = '#EF4444';
      curDot.style.transform = 'scale(1.5)';
    }
  }
